...
class Watershed:
    def markers(self, Video, frames, method, cores=1, inplace=True, **kwargs):
        if method == "isodata":
    ...
    def watershed(self, Video, frames, markers, inplace=True, cores=1, **kwargs):
        ...
        if Video is not None and markers is not None:
            raise ValueError("Provided 2 sets of frames")
        elif Video is not None and markers is None:
            markers = Video.markers
        elif Video is None and markers is None:
            raise ValueError("did not provide any markers")

        if len(frames) != len(markers):
            raise ValueError("your frames and markers are not the same length")
        if cores == 1:
            masks = []
            for i in range(len(frames)):
                mask = watershed(frames[i], markers[i], **kwargs)
                masks.append(mask)
        else:
            with Pool(cores) as p:
                watershed_func = curry(watershed, **kwargs)
                inputs = zip(frames, markers)
                masks = p.starmap(watershed_func, inputs)

        if inplace and Video is not None:
            Video.masks = masks
        elif inplace and Video is None:
            ValueError("did not specify a video class object")
        else:
            return masks

    def properties(self, masks=None, frames=None, cores=1, **kwargs):
        ...
        if cores > 1:
            with Pool(cores) as p:
                ...
                measures = p.starmap(properties, inputs)        else:
            measures = []
